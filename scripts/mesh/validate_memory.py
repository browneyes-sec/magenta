...
REQUIRED_FIELDS = [
    "text", "agent_role", "mission_id", "tenant_id",
    "memory_type", "created_at", "source_tool",
]
    ...
    try:
        ...
        if results and results[0]["id"] == test_id:
            ...
            report.add(CheckResult(
                name="roundtrip_full",
                severity=Severity.PASS,
                message=f"Write→Search round-trip OK",
                details={
...
def main():
    ...
    parser.add_argument("--write-test", action="store_true",
                        help="Run write/read round-trip test")
    ...
    if args.output == "json":
        ...
        print(f"  Pass: {summary['pass']}  Warn: {summary['warnings']}  Critical: {summary['critical']}")
        print(f"{'='*60}\n")
