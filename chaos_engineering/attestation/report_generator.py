...
class ReportGenerator:
    ...
    def generate(self, result: Any, run_id: str) -> str:
        ...
        parts = run_id.split("-")
        date_str = (
            "-".join(parts[1:4]) if len(parts) >= 4 else datetime.utcnow().strftime("%d_%m_%y")
        )
    ...
    def _render_markdown(self, result: Any) -> str:
        ...
        lines.append(
            f"**Intensity:** {result.intensity} (stealth: {'on' if result.stealth else 'off'})"
        )
        lines.append("")
        ...
        if result.regression:
            lines.append(
                f"| Regression | {result.regression.get('passed', 0)}/{result.regression.get('total', 0)} passed |"
            )

        verdict_icon = {"pass": "✅", "fail": "❌", "skip": "⏭️", "dry_run": "🔍"}.get(
            result.verdict, "❓"
        )
        ...
        for r in result.results:
            status_icon = {"passed": "✅", "failed": "❌", "skipped": "⏭️", "error": "⚠️"}.get(
                r.status, "❓"
            )
        ...
        if result.regression and result.regression.get("status") != "skipped":
            ...
            lines.append("| Mode | Total | Passed | Failed |")
            lines.append("|---|---|---|---|")
            lines.append(
                f"| {result.regression.get('mode', 'unknown')} | "
        ...
        # Footer
        lines.append("---\n")
        lines.append("*Certification generated by Magenta Chaos Engineering Suite v0.1.0*")
        lines.append("*DTP-03 §5.3 Compliance: Chaos test plan executed*")
