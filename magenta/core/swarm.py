...
class SwarmManager:
            ...
            tasks.append(
                {
                    ...
                    "role": "compliance",
                    "status": "pending",
                    "dependencies": [f"contain-{mission.mission_id[:8]}"]
                    if severity.value >= SeverityLevel.high.value
                    else [],
                }
            )
    ...
    async def execute_mission(self, mission_id: str) -> dict[str, Any]:
        ...
        if independent:
            ...
            results = await asyncio.gather(
                *[self._execute_single_task(t, mission, task_context) for t in independent],
        ...
        while remaining:
            ...
            ready = [
                t for t in remaining if all(dep in executed for dep in t.get("dependencies", []))
            ]
            if not ready:
                missing = []
                for t in remaining:
            ...
            results = await asyncio.gather(
                *[self._execute_single_task(t, mission, task_context) for t in ready],
                return_exceptions=True,
            )
    ...
    async def _execute_single_task(
        ...
        if not agent:
            raise AgentError(f"Task {task['task_id']}: agent {agent_id} not found in registry")
