...
def generate_metrics() -> str:
    ...
    for role, count in agent_counts.items():
        ...
        for agent in agents_list:
            if hasattr(agent, "role") and agent.role == role:
                ...
                lines.append(
                    f'magenta_agent_active_tasks{{role="{role}",agent_id="{agent.agent_id}"}} {active}'
                )
                lines.append(
                    f'magenta_agent_max_concurrent_tasks{{role="{role}",agent_id="{agent.agent_id}"}} {max_concurrent}'
                )
    ...
    for e in workflow_engine._executions.values():
        for node_id, node_exec in e.node_executions.items():
            ...
            if node_exec.started_at and node_exec.completed_at:
                ...
                lines.append(
                    f'magenta_workflow_node_duration_seconds_bucket{{node_type="{node_type}",le="0.5"}} 1'
                )
                lines.append(
                    f'magenta_workflow_node_duration_seconds_bucket{{node_type="{node_type}",le="1.0"}} 1'
                )
                lines.append(
                    f'magenta_workflow_node_duration_seconds_bucket{{node_type="{node_type}",le="5.0"}} 1'
                )
                lines.append(
                    f'magenta_workflow_node_duration_seconds_bucket{{node_type="{node_type}",le="10.0"}} 1'
                )
                lines.append(
                    f'magenta_workflow_node_duration_seconds_bucket{{node_type="{node_type}",le="+Inf"}} 1'
                )
                lines.append(
                    f'magenta_workflow_node_duration_seconds_sum{{node_type="{node_type}"}} {duration}'
                )
                lines.append(
    ...
    for e in workflow_engine._executions.values():
        ...
        for approval_id, approval_data in e.approvals_completed.items():
            ...
            if "latency_seconds" in approval_data:
                lines.append(
                    f'magenta_workflow_approval_latency_seconds_bucket{{decision="{decision}",le="30.0"}} 1'
                )
                lines.append(
                    f'magenta_workflow_approval_latency_seconds_bucket{{decision="{decision}",le="60.0"}} 1'
                )
                lines.append(
                    f'magenta_workflow_approval_latency_seconds_bucket{{decision="{decision}",le="300.0"}} 1'
                )
                lines.append(
                    f'magenta_workflow_approval_latency_seconds_bucket{{decision="{decision}",le="+Inf"}} 1'
                )
                lines.append(
                    f'magenta_workflow_approval_latency_seconds_sum{{decision="{decision}"}} {approval_data["latency_seconds"]}'
                )
                lines.append(
    ...
    for (subgraph, status), count in subgraph_calls.items():
        lines.append(
            f'magenta_workflow_subgraph_invocations_total{{subgraph="{subgraph}",status="{status}"}} {count}'
        )
    ...
    try:
        ...
        for client_name, cb in model_router._circuit_breakers.items():
            lines.append(
                f'magenta_circuit_breaker_open{{client="{client_name}"}} {1 if cb._is_open else 0}'
            )
            lines.append(
...
def create_app() -> FastAPI:
    ...
    async def metrics():
        ...
        return Response(
            content=generate_metrics(), media_type="text/plain; version=0.0.4; charset=utf-8"
        )
