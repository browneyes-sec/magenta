...
async def get_pending_approvals(
    ...
    return {
        ...
        "skip": skip,
        "approvals": [
            _format_request(req) for req in pending
        ],
    }
...
async def approve_action(
    ...
    import asyncio
    asyncio.ensure_future(_notify_orchestrator(
        correlation_id, "approved", decision
    ))
...
async def deny_action(
    ...
    import asyncio
    asyncio.ensure_future(_notify_orchestrator(
        correlation_id, "denied", decision
    ))
...
async def _notify_orchestrator(
    ...
    try:
        ...
        await client.send("approval-responses", {
            "correlation_id": correlation_id,
            "action": action,
            "approver_id": decision.approver_id,
