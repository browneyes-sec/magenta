...
from magenta.core.models import (
    Playbook, PlaybookV2, WorkflowNode, WorkflowEdge, WorkflowSpec,
    SubgraphSpec, GovernanceSpec, WorkflowNodeType, Mission
)
...
class WorkflowCompiler:
    ...
    def _parse_legacy_spec(self, pb: Playbook) -> WorkflowSpec:
        ...
        for i, stage in enumerate(stages):
            ...
            if role in ["ingest", "enrich", "triage"]:
                node_type = WorkflowNodeType.ingest if role == "ingest" else WorkflowNodeType.agentic
    ...
    def _compile_parallel_node(self, node: WorkflowNode, params: dict) -> DAGNode:
        return DAGNode(
            ...
            role="parallel",
            depends_on=node.depends_on,
            params={**node.config, **params, "node_type": "parallel", "branches": node.config.get("branches", [])},
        )
    ...
    def _compile_subgraph_node(self, node: WorkflowNode, params: dict) -> DAGNode:
        return DAGNode(
            ...
            role="subgraph",
            depends_on=node.depends_on,
            params={**node.config, **params, "node_type": "subgraph", "subgraph_name": node.subgraph},
        )
